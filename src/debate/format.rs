...
const CROSS_EXAM: &str =
    "[형식: 교차신문] 상대에게 날카로운 질문 하나만 던지고, 그게 왜 중요한지 설명하세요.";
const STEELMAN: &str =
    "[형식: 스틸맨 후 반박] 상대의 가장 강한 논점을 공정하게 다시 말한 뒤 전제 하나를 공격하세요.";
const CONCRETE_CASE: &str =
    "[형식: 사례] 구체적 사례 하나만 들어 입장을 세우세요. 두 번째 사례는 붙이지 마세요.";
const THRESHOLD: &str =
    "[형식: 기준] 측정 가능한 임계값이나 롤백 조건을 하나 제안하세요.";
const CONCESSION: &str =
    "[형식: 조건부 양보] 무엇이 충족되면 입장을 바꿀지 분명히 말하세요.";
const DILEMMA_FORK: &str =
    "[형식: 양자택일] 두 선택지의 트레이드오프를 세우고 상대에게 고르라고 요구하세요.";
const DIRECT_REBUTTAL: &str =
    "[형식: 직접 반박] 바로 앞 발언의 한 지점을 골라 닉네임을 부르며 반박하세요.";
const OPENING: &str =
    "[형식: 입장 표명] 자기 입장을 한 문장으로 분명히 한 뒤 근거 하나를 붙이세요.";
const PREDICTION: &str =
    "[형식: 예측] 구체적 예측과 신뢰도를 말하고, 무엇을 보면 틀린 건지 덧붙이세요.";
