...
## 그냥 페르소나 채팅이랑 뭐가 달라? · How is this different from just persona chat?
...
> In the usual "give an LLM a persona and chat" setup, everyone answers every turn — round-robin, or whoever got prompted. No sense of timing, no silence, no "I'll just listen this round."
>
...
>
> You don't script the rules. You turn a few knobs and **watch** — and personalities fall out as rhythms: the chatterbox, the quiet one, the one who only chimes in sometimes.

---

## 알고리즘, 쉽게 · How it works (the simple version)
...
> Four small parts, that's it.
> - **μ** = each persona's baseline chattiness (0–1).
> - **λ** = an urge that recovers toward μ each tick and drops right after they speak (so nobody hogs the floor). *A stripped-down Hawkes self-exciting process.*
> - **θ** = a silence gate: if no λ clears θ this tick, the room stays quiet. Higher θ → more silence.
> - **who?** = when several clear θ, fuse three signals — who wants it most (λ), who's spoken least lately (fairness), and a pinch of randomness — and pick one. *Reciprocal Rank Fusion.*
>
> No LLM needed to get a rhythm; v0.1 uses contentless fake utterances. The personality lives in the **timing**.

### 손잡이가 뭘 하는지 (실제 출력) · what the knobs do (real output)
...
> Same μ, but a single θ turns the room from nonstop, to rhythmic, to nearly silent. (Aside: spread is actually governed more by the *fairness signal* than by k — which is why changing k barely moves the numbers above. You learn things like this by watching the meter.)
...
> All you need is [Rust](https://rustup.rs). v0.1 needs no LLM and no network. Same `--seed` → identical output every run, so it's testable headless.
...
> **v0.1 (now):** the rhythm engine. No LLM yet — fake utterances let you verify the *timing* is alive before any model is involved. Deterministic, with a debug meter. **Next:** v0.2 chemistry (α — who riles up whom; room presets), then v0.3 real Ollama personas generating the actual lines while the engine still decides who speaks.
...
> Most multi-agent LLM demos solve a task and stop. Real small talk has no task and no end — it just flows and fades. That needed a different kind of engine, so here's one you can poke at.

---

*알고리즘 출처 / sources: Multivariate Hawkes process (self-exciting point process), Reciprocal Rank Fusion, AutoGen GroupChat 패턴. 자세한 설계는 [`docs/reference/salon-engine-design.md`](docs/reference/salon-engine-design.md).*
