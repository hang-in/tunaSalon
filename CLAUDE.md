...
- **다음(이후 트랙)**: ① **friend engine 심화**(BGE-M3 의미검색·SQLite 영속(L1)·망각·주관적 저장·인물기억·발화층 judge) - v0.8 위에 쌓음. 설계노트 `docs/temp/salon-memory-engine-idea.md`. ② **web 프런트엔드**(planned·파킹, `docs/plans/salon-web-frontend.md`) - "외부 사용자가 봤을 때 프로덕션 레벨 앱" 목표로 채팅 UI를 web으로. Rust 엔진 무수정 + axum **WebSocket** 새 sink(엔진 push↔사람 입력, blocking 엔진↔async 채널 브리지) + 정적 HTML/CSS/JS 1장, TUI는 디버그 강등(유지), 골든/headless 무손상, 키는 서버에만(WASM-only 불가), feature flag로 기본 빌드 무영향. 전송은 WebSocket 결정(채팅앱 정석, SSE+POST 거절). ③ `/invite`·페르소나 비주얼(persona-ui §3/§4), v0.7 발산 boost. 인터랙티브 `--chat`은 실제 터미널 검증 권장.
