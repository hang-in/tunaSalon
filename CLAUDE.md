...
- **다음(이후 트랙)**: ① **friend engine 심화 = 다음 active 트랙** - 본인 레포 seCall 검색코어 lift. **플랜 `docs/plans/salon-engine-v9.md`(planned)**: v0.9=Stage 0(Lindera 한국어 형태소)+Stage 1(SQLite 영속 MemoryStore + FTS5 BM25 회상), 전 검색코어는 `friend-engine` feature flag 뒤(기본 off→골든·기본빌드 lean), 회상은 라이브 전용→골든 보존. BGE-M3 의미검색(ORT, ollama 아님)·usearch HNSW·hybrid RRF는 Stage 2~3=v0.10. 망각·주관저장·인물기억은 tunaSalon 고유(트랙 밖). 메모리 [[secall-search-core]], 설계노트 `docs/temp/salon-memory-engine-idea.md`. ② **web 프런트엔드**(planned·파킹, `docs/plans/salon-web-frontend.md`) - "외부 사용자가 봤을 때 프로덕션 레벨 앱" 목표로 채팅 UI를 web으로. Rust 엔진 무수정 + axum **WebSocket** 새 sink(엔진 push↔사람 입력, blocking 엔진↔async 채널 브리지) + 정적 HTML/CSS/JS 1장, TUI는 디버그 강등(유지), 골든/headless 무손상, 키는 서버에만(WASM-only 불가), feature flag로 기본 빌드 무영향. 전송은 WebSocket 결정(채팅앱 정석, SSE+POST 거절). ③ `/invite`·페르소나 비주얼(persona-ui §3/§4), v0.7 발산 boost. 인터랙티브 `--chat`은 실제 터미널 검증 권장.
