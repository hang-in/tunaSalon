...
- **다음(번호 로드맵 완료 후 이후 트랙)**: 가장 유력 = **장기기억(friend engine)** - 페르소나가 사용자와 지난 대화 기억(회상 슬롯 v0.3 `assemble_user_prompt`에 예약). 설계노트 `docs/temp/salon-memory-engine-idea.md`. 그 외: BGE-M3(FlowMeter 정밀도), `/invite`·페르소나 비주얼, v0.7 발산 boost. 인터랙티브 `--chat`은 실제 터미널에서 사용자 검증 권장.
