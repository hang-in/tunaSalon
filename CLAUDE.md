...
개발은 Architect(계획)·Developer(구현)·Reviewer(검토) 역할 분리로 진행하며(`docs/agents/`), 실제 구현은 **Sonnet 서브에이전트에 위임**(codex 비사용, 2026-06-02 사용자 지시)하고 Claude(Opus)가 스펙 작성·리뷰·검증합니다. **현재 v0.1~v0.4 구현 완료**(2026-06-02): 리듬 엔진(v0.1) + 케미 α(v0.2) + 로컬 LLM PersonaRuntime(v0.3) + 동시 호출·이종 백엔드 풀(v0.4). 125 tests, 스모크 게이트 4종 green, 공개 레포 https://github.com/hang-in/tunaSalon. 다음은 v0.5(FlowMeter). 단계 로드맵은 `docs/plans/salon-engine-v1.md`, 단계별 플랜 `salon-engine-v2.md`·`v3`·`v4` 전부 done.
...
Rust 프로젝트. `src/` 모듈: model, sink, hawkes, gate, rrf, utterance, driver, headless, tui, sweep, preset, runtime, ollama, openai, pool, semaphore.

```bash
cargo test                                       # 전체 125 tests (스모크 게이트 4종: smoke=v0.1, smoke_v2, smoke_v3, smoke_v4)
...
- **v0.5 (FlowMeter)**: 대화 수렴/발산 측정. 처음부터 BGE-M3 가지 말고 키워드 중복/문장 유사도 근사로 시작, 관찰만(피드백은 v0.6 MetaController). 로드맵 `docs/plans/salon-engine-v1.md` §3.
- **사람 참여(설계 완비, 미구현)**: 설계 SSOT `salon-engine-design.md` **§5**가 코어 메커니즘 명세 — 사람 = Hawkes 외부 이벤트(강도 상시 무한), 발화 시 **큰 mark**로 전 페르소나 λ 강자극 + 강도 일부 리셋 + **화제 선점**(관심도/수렴 측정 기준점). §7에 `HumanChannel`(입력층, **코어** 모듈), §8 "사람 입력은 어느 틱에든 인터럽트". 데이터 모델은 이미 호환(`Event.speaker` 자유 문자열, examples가 "사람" opening 사용)이나 **HumanChannel 미구현**(stdin/TUI 입력 경로 없음, driver 자율 선택). 구현 = HumanChannel + 틱 인터럽트(큰 mark→history+λ 자극·리셋+화제 갱신) + 인터랙티브 입력. 비결정이라 LLM-off 골든 경로 불침투(opt-in). 메모리 [[human-participation]].
