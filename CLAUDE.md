...
- **v0.5 (사람 참여 채팅방, P0)**: 제품 되찾기 - HumanChannel(design §5: 큰 mark 이벤트→전 λ 자극+화제 선점) + 채팅 TUI(persona-ui §5) + 비블로킹 라이브 드라이버(생성 워커+채널, ~1.6s 지연이 UI 안 막게). headless는 dev 회귀 도구로 강등. 플랜 `salon-engine-v5.md`, task 28~32.
- **v0.6 (FlowMeter)**: 대화 수렴/발산 측정(키워드/유사도 근사 → BGE-M3), 관찰만. v0.7 MetaController(거시 피드백). (재정렬로 FlowMeter가 v0.5→v0.6 이동.)
