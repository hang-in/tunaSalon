...
개발은 Architect(계획)·Developer(구현)·Reviewer(검토) 역할 분리로 진행하며(`docs/agents/`), 실제 구현은 **Sonnet 서브에이전트에 위임**(codex 비사용, 2026-06-02 사용자 지시)하고 Claude(Opus)가 스펙 작성·리뷰·검증합니다. **현재 v0.1~v0.9 구현 완료**(2026-06-03): 리듬(v0.1) + 케미 α(v0.2) + 로컬 LLM(v0.3) + 동시 호출·이종 백엔드 풀(v0.4) + 사람 참여 채팅방(v0.5) + FlowMeter(v0.6) + MetaController(v0.7) + friend engine 첫 증분(v0.8) + **friend engine 심화(v0.9: 한국어 형태소 Lindera + SQLite/FTS5 BM25 회상 + 세션 넘는 영속, seCall 검색코어 lift, `friend-engine` feature 뒤·골든 무손상) + 생동감 `--chat`(튜닝된 3-way config + `/topic`))**. 222 tests(feature면 230), 스모크 게이트 green, 공개 레포 https://github.com/hang-in/tunaSalon. v2~v9 done. **다음 이후 트랙**: **v0.10 = friend engine Stage 2**(BGE-M3 ONNX 의미검색·usearch HNSW·hybrid BM25/벡터 융합, 이후 망각·주관저장·인물기억) / **web 프런트엔드**(planned, `docs/plans/salon-web-frontend.md`: axum WebSocket, Kimi UI 디자인 위임 중 `docs/temp/salon-web-ui-kimi-prompt.md`) / **페르소나 합성 + 픽셀아트 캐릭터**(MBTI/혈액형/별자리/역할 프리셋 조합, 싸이월드 미니미 풍, persona-ui §2~4) / v0.7 boost.
...
```bash
cargo test                                       # 전체 222 tests (friend-engine feature면 230). 스모크 + recall_eval + smoke_chat(3-way 생동감) + smoke_v9
...
- **다음(이후 트랙)**: ① **v0.10 = friend engine Stage 2** - BGE-M3 임베딩(seCall `embedding.rs` ORT/ONNX, **로컬 ollama 금지→ORT in-process+CoreML**) + usearch HNSW(`ann.rs`) + hybrid RRF(`hybrid.rs`, BM25+벡터 융합). 난제: recall_eval 결정성(임베딩 float 비결정→고정모델+허용오차 or mock). 이후 망각·주관저장·인물기억. ② **web 프런트엔드**(planned, `docs/plans/salon-web-frontend.md`: axum **WebSocket** 새 sink, blocking 엔진↔async 브리지, 정적 프런트, TUI 디버그 강등, 골든 무손상, 키 서버에만/WASM 불가, feature flag). **UI 디자인은 Kimi 2.6에 위임 중**(`docs/temp/salon-web-ui-kimi-prompt.md`). ③ **페르소나 합성 + 픽셀아트 캐릭터** - MBTI/혈액형/별자리/역할 프리셋 조합으로 페르소나·아바타 생성(싸이월드 미니미 풍, 웹 렌더). persona-ui §2(40조각)·§3(런타임 조립)·§4(비주얼층). "친구 만들기"=/invite. ④ v0.7 발산 boost. 인터랙티브 `--chat`은 실제 터미널 검증 권장.
