...
- **사람 참여(future, 미구현)**: `Event.speaker`는 자유 문자열이라 사람 한 턴 = history에 Event push로 표현 가능하고 페르소나가 반응함(examples의 "사람" opening). 단 **라이브 입력 경로 없음**(stdin/TUI 입력/`/invite` 미구현, driver는 자율 선택). 구현하려면 틱 루프에 사람 턴 주입 + 인터랙티브 입력 추가. 별도 트랙(design 노트 `/invite` = "이후").
