...
- **다음(이후 트랙)**: ① **task-50 = v0.10 마감**(새 세션) - 실 `OrtEmbedder`를 라이브 `live_store`(memory.rs)에 배선(모델 있으면 Ort/없으면 Mock 폴백; **임베더는 DB당 일관해야** - Mock/Ort 혼용 금지, 바뀌면 ANN 재구축 필요) + 실모델 의미 `#[ignore]` 테스트(어휘≠의미 SSOT 회상) + smoke_v10 + README/CLAUDE/index v0.10 bump. **★ task-50 전에 리팩토링 리뷰 먼저**: `docs/reference/refactoring-review-v9-snapshot.md`(**이미 작성됨**, pi/MiniMax-M3, 27KB) 읽고 제안 중 골든/feature off-on 불변식에 안전한 정리만 적용(큰 재설계는 보류). ② **web 프런트엔드**(`docs/plans/salon-web-frontend.md`: axum WebSocket, Kimi UI 초안 `web/`→데이터 계약 맞춰 배선). ③ **페르소나 합성+픽셀아트 캐릭터**(persona-ui §2~4, 싸이월드 미니미, "친구 만들기"=/invite). ④ v0.7 발산 boost. 인터랙티브 `--chat`은 실제 터미널 검증 권장.
