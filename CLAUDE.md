...
개발은 Architect(계획)·Developer(구현)·Reviewer(검토) 역할 분리로 진행하며(`docs/agents/`), 실제 구현은 **Sonnet 서브에이전트에 위임**(codex 비사용, 2026-06-02 사용자 지시)하고 Claude(Opus)가 스펙 작성·리뷰·검증합니다. **현재 v0.1~v0.10 구현 완료**(2026-06-03): 리듬(v0.1) + 케미 α(v0.2) + 로컬 LLM(v0.3) + 동시 호출·이종 백엔드 풀(v0.4) + 사람 참여 채팅방(v0.5) + FlowMeter(v0.6) + MetaController(v0.7) + friend engine 첫 증분(v0.8) + **friend engine 심화(v0.9: 한국어 형태소 Lindera + SQLite/FTS5 BM25 회상 + 세션 넘는 영속, seCall 검색코어 lift, `friend-engine` feature 뒤·골든 무손상) + 생동감 `--chat`(튜닝된 3-way config + `/topic`))**. + **friend engine 의미검색 v0.10 완료(task 47~50)**: `OrtEmbedder`(BGE-M3 ONNX **in-process**, `embed.rs`. **실측 viable**: 로드 3.8s/embed 29ms/2.3GB, ollama 아님·랙 없음, ort=download-binaries) + usearch HNSW(`ann.rs`) + hybrid RRF 회상(BM25 어휘+벡터 의미, 참여격리) + 실 OrtEmbedder를 라이브 `live_store`에 배선(모델 있으면 Ort/없으면 Mock 폴백, `meta.embedder_kind`로 DB당 일관·불일치 시 reembed_all로 전체 재구축). 새 `friend-engine-semantic` feature 뒤(기본/골든 무손상, recall은 cfg로 v0.9 보존). 일상 테스트는 MockEmbedder 결정적, 실모델은 `#[ignore]`(어휘≠의미 회상 검증·통과: 쿼리 "반려동물 데리고 나갔어"→"강아지랑 산책 다녀왔어" rank 1). default 226 / friend-engine 235 / friend-engine-semantic 263 tests, 골든 5/5 무손상. 리팩토링: FLOW_WINDOW 상수 단일화(driver/live 중복 제거, refactoring-review-v9-snapshot.md 6-1 적용·골든 무손상). 공개 레포 https://github.com/hang-in/tunaSalon. v2~v10 done. **web 제품 트랙 대거 진행(2026-06-03)**: 동적 persona 초대(add/remove_persona + persona_meta + pool.generate_on + 케미 alpha 동적 재계산) + 단일방 영속(rooms.db, web 독립) + 형태소 flow(morphology feature, 한국어 흐름 게이지) + 활기 게이지(liveliness) + 4축 배지 + pace 런타임 조절 + LLM qwen3.6-35b-fast/thinking off(vllm-swap 주의) + base_rate 라이브 튜닝. `--web`으로 사용자가 직접 테스트 중. plan `docs/plans/salon-web-ux.md`. **다음**: ① **영속 4축 저장 + 복원 재assemble**(최우선: 코드 변경 자동 반영 + 4축 배지 유지, rooms.db 매번 비우는 번거로움 해결). ② base_rate 라이브 튜닝 계속. ③ 멀티룸 · 본인 프로필/프리셋 8개. ④ 웹서치(tool use). ⑤ 픽셀아트 캐릭터. **상세 핸드오프 `docs/temp/next-session-prompt.md`**.
...
- **v0.10 (friend engine Stage 2 = 의미검색) - 완료(task 47~50)**: 플랜 `docs/plans/salon-engine-v10.md`. 2a(task-47) Embedder/Mock/Ort BGE-M3 `embed.rs`(ORT 실측 viable 3.8s/29ms/2.3GB, `download-binaries`, [[ort-embedding-viable]]) / 2b(task-48) usearch HNSW `ann.rs` + `memory_vectors` BLOB + `vector_search` / 2c(task-49) hybrid RRF 회상(BM25 어휘+벡터 의미, 양 leg 참여격리, **v0.9 recall은 cfg로 보존**, RRF k=60) / 2d(task-50) 실 OrtEmbedder를 `live_store`에 배선(모델 있으면 Ort/없으면 Mock 폴백) + `meta.embedder_kind`로 DB당 임베더 일관(불일치 시 `reembed_all`로 전체 재임베딩·과거 사건 백필) + 실모델 의미 `#[ignore]` 테스트(어휘≠의미 통과) + smoke_v10. 전부 `friend-engine-semantic` feature 뒤, 일상 테스트는 MockEmbedder(결정적). default 226/friend-engine 235/semantic 263, 골든 5/5 무손상. seCall 검색코어 lift([[secall-search-core]]).
- **다음(이후 트랙)**: ① **web 프런트엔드**(`docs/plans/salon-web-frontend.md`: axum WebSocket, Kimi UI 초안 `web/`→데이터 계약 맞춰 배선). ② **페르소나 합성+픽셀아트 캐릭터**(persona-ui §2~4, 싸이월드 미니미, "친구 만들기"=/invite). ③ **friend engine 추가**(망각·캐릭터별 주관적 저장·방을 가로지르는 인물 인상, v0.10 스토어 위에). ④ v0.7 발산 boost. **리팩토링 다음 후보**(보류분, refactoring-review-v9-snapshot.md): driver/live per-tick 알고리즘 통합(`decide_one_tick` 추출, 2-1, 중간 리스크·결정성 민감→별도 세션 권장) + `live_store` side-effect 격리(단 main.rs가 lib free fn을 호출해 `pub(crate)` 불가, 대안 필요). v0.10에서는 FLOW_WINDOW 상수 단일화만 안전 적용함. 인터랙티브 `--chat`은 실제 터미널 검증 권장.
