...
상세 플랜 `salon-engine-v5.md`. 제품의 실체는 사용자가 LLM 페르소나들과 떠드는 채팅방인데, v0.1~v0.4가 엔진·headless·동시성에 집중하다 정작 채팅 화면·사람 참여를 못 만들고 실험 하네스로 드리프트했다(2026-06-02 재정렬). v0.5는 **HumanChannel**(design §5: 사람 발화 = 큰 mark Hawkes 이벤트 → 전 λ 강자극 + 화제 선점) + **채팅 TUI**(persona-ui §5: 채팅 pane + 게이지 사이드바 + 입력창) + **비블로킹 라이브 드라이버**(생성 워커 + 채널, ~1.6s 지연이 UI 안 막게)로 사람을 1급 참여자로 앉힌다. headless/스모크는 엔진 회귀 검증용 dev 도구로 강등(유지). 엔진(v0.1~v0.4)은 버리는 게 아니라 채팅방의 생동감으로 재배치된다.
