...
| v0.4 | 동시 호출이 안정적인가 | 백엔드 풀, max_concurrent, 큐·타임아웃 | 2~3명 |
| **v0.5** | **사람이 방에 들어가는가 (제품 되찾기)** | **HumanChannel(design §5) + 채팅 TUI(persona-ui §5) + 비블로킹 라이브 드라이버** | 사람+2~3명 |
| v0.6 | 대화가 식는 걸 잡는가 | FlowMeter(키워드/유사도 근사 → BGE-M3) | 2~3명 |
| v0.7 | 거시 피드백이 안정적인가 | MetaController(약하게) | 2~3명 |
| 이후 | 페르소나가 기억하는가 | 장기기억(friend engine), 회상 슬롯 v0.3 예약됨 | 사람+2~3명 |
...
### v0.5 — 사람 참여 채팅방 (제품 되찾기)
상세 플랜 `salon-engine-v5.md`. 제품의 실체는 사용자가 로컬 LLM 페르소나들과 스몰토크하는 채팅방인데, v0.1~v0.4가 엔진·headless·동시성에 집중하다 정작 채팅 화면·사람 참여를 못 만들고 실험 하네스로 드리프트했다(2026-06-02 재정렬). v0.5는 **HumanChannel**(design §5: 사람 발화 = 큰 mark Hawkes 이벤트 → 전 λ 강자극 + 화제 선점) + **채팅 TUI**(persona-ui §5: 채팅 pane + 게이지 사이드바 + 입력창) + **비블로킹 라이브 드라이버**(생성 워커 + 채널, ~1.6s 지연이 UI 안 막게)로 사람을 1급 참여자로 앉힌다. headless/스모크는 엔진 회귀 검증용 dev 도구로 강등(유지). 엔진(v0.1~v0.4)은 버리는 게 아니라 채팅방의 생동감으로 재배치된다.

### v0.6 — FlowMeter (구현 완료, 2026-06-02)
수렴/발산을 측정. 토큰 중복 근사로 시작(`src/flow.rs`, BGE-M3는 measure 인터페이스 유지한 채 이후). 관찰만 - 엔진 피드백은 안 건다(그건 v0.7). record + 채팅 사이드바 게이지, content 게이팅으로 골든 보존. 상세 `salon-engine-v6.md`. (재정렬로 v0.5→v0.6 이동. 화제 선점과 맞물려 채팅방 다음에 자연스럽다.)

### v0.7 — MetaController (구현 완료, 2026-06-02)
거시→미시 피드백(수렴 높으면 base rate μ를 낮춰 식힘)을 약하게 켠다. `src/meta.rs` cooling→mu_scale, driver/live의 update_intensities에 적용. floor 0.4·약한 게인(SALON_META_GAIN)으로 고착/진동 방지, content 게이팅으로 골든 보존. 상세 `salon-engine-v7.md`. 이 피드백 루프가 가장 불안정한 부분이라 맨 마지막·가장 약하게. **이로써 원래 단계 로드맵(v0.1~v0.7) 엔진층 완성.** 이후는 장기기억(friend engine) 등 별도 트랙.

### 이후 — 장기기억 (friend engine)
페르소나가 사용자와의 지난 대화를 기억한다. 회상 슬롯은 v0.3 `assemble_user_prompt`에 예약됨(recall=None). 설계 노트 `docs/temp/salon-memory-engine-idea.md`·`salon-recall-eval-harness.md`·`salon-context-prompt.md`. 채팅방(v0.5) 위에서 특히 가치가 크다(사용자를 기억하는 방).
