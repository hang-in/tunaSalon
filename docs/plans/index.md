...
| slug | 상태 | 버전 | 요약 |
|------|------|------|------|
| [salon-web-frontend](salon-web-frontend.md) | planned | v0.9 | **파킹(착수 전, 결정 기록)**. "프로덕션 레벨 앱" 목표로 채팅 UI를 web으로. Rust 엔진 그대로 + axum **WebSocket** 새 sink(엔진 push↔사람 입력, blocking 엔진↔async 채널 브리지) + 정적 HTML/CSS/JS 1장. TUI는 디버그 sink로 강등(유지). golden/headless 무손상, 키는 서버에만(WASM-only 불가). feature flag로 기본 빌드 무영향 |
