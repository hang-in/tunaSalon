...
| slug | 상태 | 버전 | 요약 |
|------|------|------|------|
| [salon-engine-v10](salon-engine-v10.md) | done | v0.10 | **구현 완료**(friend engine Stage 2 = 의미 회상, task 47~50). 2a(47) Embedder/Mock/Ort BGE-M3(실측 viable 3.8s/29ms/2.3GB, download-binaries) / 2b(48) usearch HNSW + memory_vectors + vector_search / 2c(49) hybrid RRF(BM25+벡터, 참여격리, v0.9 recall cfg 보존) / 2d(50) 실 OrtEmbedder를 live_store 배선(모델 있으면 Ort/없으면 Mock 폴백) + meta.embedder_kind DB당 일관(불일치 시 reembed_all 재구축) + 실모델 의미 #[ignore] 테스트 + smoke_v10. `friend-engine-semantic` feature 뒤, 일상 테스트는 MockEmbedder 결정적. default 226/friend-engine 235/semantic 263, 골든 5/5 무손상. 실모델 #[ignore] 통과(어휘 != 의미 회상) |
