...
| slug | 상태 | 버전 | 요약 |
|------|------|------|------|
| [salon-engine-v9](salon-engine-v9.md) | planned | v0.9 | **friend engine 심화 1단계**(active 다음). 본인 레포 seCall 검색코어 lift. v0.9=Stage 0(Lindera 한국어 형태소) + Stage 1(SQLite 영속 MemoryStore + FTS5 BM25 회상). 전 검색코어는 `friend-engine` feature flag 뒤(기본 off→골든·기본빌드 lean). 회상은 라이브 전용→골든 보존. BGE-M3/usearch/hybrid RRF는 Stage 2~3=v0.10. task 43~46 |
