...
| slug | 상태 | 버전 | 요약 |
|------|------|------|------|
| [salon-engine-v10](salon-engine-v10.md) | in_progress | v0.10 | **friend engine Stage 2 = 의미 회상. 2a/b/c 완료, task-50(마감) 미완**. 2a(47) Embedder/Mock/Ort BGE-M3(실측 viable 3.8s/29ms/2.3GB, download-binaries) / 2b(48) usearch HNSW + memory_vectors + vector_search / 2c(49) hybrid RRF(BM25+벡터, 참여격리, v0.9 recall cfg 보존). `friend-engine-semantic` feature 뒤, MockEmbedder 결정적. default 225/friend-engine 234/semantic 260, 골든 무손상. **다음 세션: 리팩토링 리뷰(refactoring-review-v9-snapshot.md) → task-50(실 Ort 배선+의미 #[ignore]+smoke_v10+v0.10 bump)** |
