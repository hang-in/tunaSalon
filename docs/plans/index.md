...
| [salon-engine-v8](salon-engine-v8.md) | in_progress | v0.8 | **friend engine(장기기억) 첫 증분**: 참여 기반 기억(사건+참여, 같은 방 캐릭터만 회상) + 키워드 회상(토큰 중복, 인메모리) + v0.3 회상 슬롯 주입 + **회상 평가 하네스**(SSOT/distractor 자동 채점). BGE-M3/SQLite/망각은 이후. content 게이팅으로 골든 보존. task 39~42 |
