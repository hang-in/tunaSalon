...
>
> **진행(2026-06-03): task F(`98804b4`)·G(`5a1ca7d`) 구현·단위검증 완료.** RoomStore(`rooms.db` 별 파일) + restore_history + web 배선(시작 시 load 복원/없으면 기본 3명, run_engine 주기 5s+shutdown save). 골든 5/5 무손상, default 205/web 223 green. **라이브 복원(--web 재시작 후 이어가기)은 사용자 검증 권장.** task H(명시 나가기 버튼)는 주기 저장으로 충분하면 생략.
